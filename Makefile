...
QHULL_OUT    := python/matplotlib/_qhull.$(PY_EXT)
...
build-ext:
	cargo build -p matplotlib-agg -p matplotlib-ft2font -p matplotlib-image -p matplotlib-qhull
	...
	cp target/debug/$(DYLIB_PREFIX)matplotlib_qhull.$(DYLIB_EXT) $(QHULL_OUT)
